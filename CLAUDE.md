...
- **Director owns everything narrative.** Canon, dice, prose. Mutations are tool calls; the engine validates each call and returns errors inline so the Director self-corrects.
...
- **Per-field mutability** (see PLAN.md). Entity `description`s and full `Event`s are write-once; `Thread.description`, `Item.description`, `Character.location_id`/`stats`, `PlayerState.location_id`/`stats`/`items` are mutable via dedicated tools. `log` is append-only.
